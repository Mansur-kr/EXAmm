...
function renderMovies(kino) {
    ...
    if (kino.length === 0) {
        ...
        kino.slice(0, 3700).forEach(objectlar => {
            const newItem = document.createElement("li");

            newItem.className =
                "w-[244px] bg-[#333333] text-white rounded flex flex-col text-center p-[20px] items-center gap-[20px]";

            newItem.innerHTML = `
                <img src="./1200x675mf.jpg.png" alt="${objectlar.Title}">
                ...
                <button class="bg-[#ff0000] text-[#fff] rounded px-[20px] py-[10px]">
                    Add to watchlist
                </button>
            `;
